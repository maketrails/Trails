 ...
 * scratch — one row per device, keyed by the device id, none for a device that was
 * never rebuilt.
 *
 ...
 */
class TrackRebuild(id: EntityID<Uuid>) : UuidEntity(id) {
    companion object : UuidEntityClass<TrackRebuild>(TrackRebuilds)

    var rebuiltAt by TrackRebuilds.rebuiltAt
}
 ...
 * to existing ones.
 */
object TrackRebuilds : IdTable<Uuid>("track_rebuild") {
    override val id = reference("device", Devices, onDelete = ReferenceOption.CASCADE)
